...
:root{
  ...
  --card:#0b1729;
  --card-b:rgba(59,130,246,.14);
  --card-bh:rgba(59,130,246,.35);

  --accent:#2563eb;
  --accent2:#60a5fa;
  --accent-d:rgba(37,99,235,.14);
...
[data-theme="light"]{
  ...
  --card:#ffffff;
  --card-b:rgba(37,99,235,.15);
  --card-bh:rgba(37,99,235,.35);

  --accent:#2563eb;
  --accent2:#1d4ed8;
  --accent-d:rgba(37,99,235,.08);
...
[data-theme="light"] .cp-input-full{
...
.cp-quota-inputs .cp-input-full{

flex:1

}
.cp-quota-inputs select.cp-input-full{
...
.proto-card.active .proto-card-check{
...
.proto-card.active .proto-card-icon{
...
0 12px 28px rgba(139,92,246,.48)
...
.conn-hero-tile:nth-child(2) .conn-hero-icon{
...
.conn-hero-tile:nth-child(3) .conn-hero-icon{
...
.conn-hero-tile:nth-child(4) .conn-hero-icon{
...
font-size:17px;
box-shadow:0 5px 14px rgba(16,185,129,.25);
}
...
[data-theme="light"] .conn-stat-box{
...
.sub-card-lock-badge.locked{
background:var(--amber-bg);
color:var(--amber-t)
}


.sub-card-lock-badge.open{
background:var(--green-bg);
color:var(--green-t)
}
...
[data-theme="light"] .sub-card-stats{
...
.modal-v2-icon{
    ...
    height:44px;
    border-radius:14px;
    background:linear-gradient(135deg,var(--purple),#7048d8);
    display:flex;
    align-items:center;
    justify-content:center;
    color:#fff;
...
[data-theme="light"] .modal-v2-input{
...
[data-theme="light"] .modal-v2-input:focus{
    background:#fff;
}

.modal-v2-input:focus~i{
    color:var(--purple);
}
.modal-v2-hint{
...
.modal-v2-btn-submit{
    ...
    align-items:center;
    gap:7px;
    box-shadow:0 7px 20px rgba(139,92,246,.35);
    transition:.18s;
}

.modal-v2-btn-submit:hover{
    transform:translateY(-2px);
    box-shadow:0 12px 26px rgba(139,92,246,.45);
}

.modal-v2-btn-submit:active{
    transform:translateY(0) scale(.98);
}

/* ══════ مودال انتخاب کانفیگ - سبک سنایی ══════ */
...
[data-theme="light"] .lmodal-search input{
    background:#fff;
}
...
.lrow-v2.checked .lrow-v2-check i{
    opacity:1;
    transform:scale(1);
}
...
.lrow-v2.checked .lrow-v2-avatar{
    background:var(--accent);
    color:#fff;
}
...
/* ══════ کانفیگ‌ها - طراحی ردیفی حرفه‌ای V2 ══════ */
...
.cfg-card.is-off .cfg-status-dot{
background:var(--red);
box-shadow:0 0 0 4px var(--red-bg)
}


.cfg-card.is-exp .cfg-status-dot{
...
<section class="pg on" id="pg-overview">
  ...
  <div class="dash-footer">
    <span class="df-text">SpareVpn RVG Gateway v10.2 · Railway · 2026</span>
    <a class="df-link" href="https://t.me/CodeBoxo" target="_blank"><i class="ti ti-brand-telegram"></i> t.me/SpareVpn</a>
  </div>
</section>
